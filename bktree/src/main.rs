...
#[doc="
Input:
Constructs a BK Tree using the words in the given training file. Words in the training file 
...
find the word with the least Levenshtein distance between it and the input word. In the case 
of a tie, the word with the highest frequency in the training file is chosen. If no words are 
found within a Levenshtein distance of 2, the program is unable to make a suggestion.
...
Note that the behavior of the spelling corrector is slightly different from the Norvig corrector 
because of the use of Levenshtein distance rather than edit distance. In particular, transposing 
letters adds 2 to the Levenshtein distance, whereas it only adds 1 to the edit distance. Thus, in 
the case where the Norvig corrector suggests a correction of transposing two letters for an edit 
distance of 1, the BK tree corrector might suggest a different correction with a Levenshtein 
distance of 1. For example, if the misspelled word is 'wordl' and 'world' appears more frequently 
in the training file than 'word', the Norvig corrector will suggest 'world' over 'word' because
they are each edit distance 1 and world is more frequent. On the other hand, the BK tree corrector
will suggest 'word' over 'world' because the Levenshtein distance between 'wordl' and 'word' is 1 
but the Levenshtein distance between 'wordl' and 'world' is 2.
"]
